...
namespace VolvoWrench.ValvePak
{
    public class Package : IDisposable
    {
        ...
        public void Read(string filename)
        {
            ...
            var fs = new FileStream($"{FileName}{(IsDirVPK ? "_dir" : string.Empty)}.vpk", FileMode.Open,
        ...
        public void Read(Stream input)
        {
            if (FileName == null)
                throw new InvalidOperationException(
                    "If you call Read() directly with a stream, you must call SetFileName() first.");

            Reader = new BinaryReader(input);

            if (Reader.ReadUInt32() != MAGIC) throw new InvalidDataException("Given file is not a VPK.");
        ...
        public PackageEntry FindEntry(string directory, string fileName, string extension)
        {
            ...
            if (!Entries.ContainsKey(extension)) return null;
        ...
        public void ReadEntry(PackageEntry entry, out byte[] output, bool validateCrc = true)
        {
            ...
            if (entry.SmallData.Length > 0) entry.SmallData.CopyTo(output, 0);
            ...
            if (entry.Length > 0)
            {
                ...
                {
                    if (entry.ArchiveIndex != 0x7FFF) fs?.Close();
                }
            }
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    while (true)
                    {
                        ...
                        if (Reader.ReadUInt16() != 0xFFFF) throw new FormatException("Invalid terminator.");

                        if (entry.SmallData.Length > 0) Reader.Read(entry.SmallData, 0, entry.SmallData.Length);
        ...
        public void VerifyHashes()
        {
            ...
            if (!IsSignatureValid()) throw new InvalidDataException("VPK signature is not valid.");
        }
        ...
        private void ReadArchiveMD5Section()
        {
            ...
            if (ArchiveMD5SectionSize == 0) return;

            var entries =
                ArchiveMD5SectionSize / 28; // 28 is sizeof(VPK_MD5SectionEntry), which is int + int + int + 16 chars
        ...
        private void ReadOtherMD5Section()
        {
            if (OtherMD5SectionSize != 48)
                throw new InvalidDataException(
                    $"Encountered OtherMD5Section with size of {OtherMD5SectionSize} (should be 48)");
        ...
        {
            if (SignatureSectionSize == 0) return;
